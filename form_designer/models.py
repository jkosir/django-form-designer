...
class FormDefinition(models.Model):
        ...
        if self.mail_replay:
            ...
            if not self.replay_attachments_field:
                if self.replay_attachments0:
                    replay_message.attach_file(self.replay_attachments0.path)
    ...
    def export_xcl(self, qs):
        ...
        fields_in_results = qs[0].form_definition.get_field_dict_include_result()
        ...
        if include_header:
            ...
            for field in fields_in_results.keys():
                header.append(field)
...
class FormLog(models.Model):
    ...
    _data = None
    #model crossing old version, change in db
    #data_old = PickledObjectField(_('Data'), null=True, blank=True)
